    ...
    def partition(self):
        ...
        for d in range(1, max_len):
            for i in range(max_len - d):
                ...
                before_lse_2 = beta[:, i + 1 : j + 1, j].view(
                    batch_size, d, 1, label_size
                )
...
class TreeCRFLayer(nn.Module):
    ...
    def forward(self, log_potentials, mask, lengths):
        ...
        if self.use_vanilla_crf:
            TreeCRF_ = TreeCRFVanilla
        else:
            TreeCRF_ = TreeCRF
        ...
        # State dropout
        if self.dropout > 0.0:
            dropout_dist = Categorical(torch.tensor([self.dropout, 1.0 - self.dropout]))
            ...
            dropout_mask_observed = dropout_mask_observed.unsqueeze(-1).repeat(
                1, 1, 1, self.observed_label_size
            )
            ...
            if self.dropout_mode == "full":
                ...
                dropout_mask_latent[:, :, :, : self.observed_label_size] = 1.0
...
class BiAffine(nn.Module):
    ...
    def forward(self, sent_states):
        ...
        sent_states_sum = (sent_states_sum_0 + sent_states_sum_1).view(
        ...
        log_potentials_1 = log_potentials_1.view(
            batch_size, max_len, max_len, label_size
        )
...
class PartialPCFG(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if self.use_crf is False:
            assert config.latent_label_size == 1
        ...
        if config.parser_type == "bilinear":
            ...
            raise NotImplementedError(
                "illegal parser type %s not implemented!" % config.parser_type
            )
    ...
    def forward(
        ...
        gather_ids = (
            gather_ids.reshape(batch_size * sequence_length, -1)
            .repeat(1, hidden_size)
            .reshape(sequence_output.shape)
        )
        gather_output = sequence_output.gather(
            1, gather_ids
        ...
        if self.use_crf is False:
            ...
            log_prob_sum_partial, log_prob_smooth, entropy, inspect_ = self.tree_crf(
                log_potentials, partial_masks, lengths
            )
    ...
    def infer(
        self, input_ids, token_type_ids, attention_mask, gather_ids, gather_masks
    ):
        label_size = self.label_size
...
class Processor(DataProcessor):
    ...
    def convert_examples_to_features(self, examples, max_seq_length, tokenizer):
        ...
        for ex_index, example in enumerate(tqdm(examples)):
            ...
            partial_masks = self.generate_partial_masks(
                example.text_a.split(" "), max_seq_length, example.label, self.labels
            )
    ...
    def generate_partial_masks(self, tokens, max_seq_length, labels, tags):
        ...
        mask = [
            [[2 for x in range(total_tags_num)] for y in range(max_seq_length)]
            for z in range(max_seq_length)
        ]
        ...
        for start, end, tag in label_list:
            ...
            for i in range(l):
                ...
                for j in range(i, l):
                    if j < start:
                        continue
                    if (i > start and i <= end and j > end) or (
                        i < start and j >= start and j < end
                    ):
                        for k in range(total_tags_num):
                            mask[i][j][k] = 0
...
class InferenceProcessor:
    ...
    def sentences_to_features(self, sentences):
        ...
        for sentence in tqdm(sentences, desc="Tokenizing"):
            ...
            partial_masks = [
                [[0 for _ in range(total_tags_num)] for _ in range(self.max_seq_length)]
...
def extract_entities_from_outputs(
    outputs,
    input_ids,
    gather_masks,
    log_potentials,
    ...
    for idx, (output, l) in enumerate(zip(outputs, gather_masks)):
        ...
        all_predictions.append({"sentence": sentence, "entities": entities})
...
def predict(model, tokenizer, processor, input_file, device, batch_size=8):
    ...
    all_gather_masks = torch.tensor(
        [f.gather_masks for f in features], dtype=torch.long
    )
    all_partial_masks = torch.tensor(
        [f.partial_masks for f in features], dtype=torch.long
    )
...
def build_knowledge_graph(predictions, output_file="knowledge_graph.json"):
    ...
    for pred in predictions:
        ...
        kg["sentences"].append(
            {"text": pred["sentence"], "entities": sentence_entities}
        )
...
def process_paper_text(
    paper_text, model, tokenizer, processor, device, grobid_url, batch_size=8
):
...
def generate_annotations(paper_text):
    ...
    tokenizer = BertTokenizer.from_pretrained(
        "bert-base-uncased", do_lower_case=False, do_basic_tokenize=False
    )
    ...
    response = process_paper_text(
        paper_text, model, tokenizer, processor, device, grobid_url, batch_size
    )
